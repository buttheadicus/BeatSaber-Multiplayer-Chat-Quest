...
if ($psVersion -ge 6) {
    ...
    Write-Output "Note: skipping qmod.schema.json validation (needs PowerShell 6+); checking JSON parses."
    try {
        $null = Get-Content $mod -Raw | ConvertFrom-Json
    }
    catch {
        Write-Output "Error: mod.json is not valid JSON."
        exit 1
    }
}
exit 0
